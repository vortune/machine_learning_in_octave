...
在这里，我们采用 GNU 开源社群的数学工具 Octave 来加载与调制训练用的图像数据。其他的训练用数据可以参考这个章节的内容来加载。
...
我们完全可以创建一个 28 \* 28 \* 60000 的 3 维数组将数据逐个地循环填充进去。 不过，在这里我们不打算这样做，而是准备利用 octave 内置的数组与矩阵操作函数来实现数据格式的调制。
...
还记得官方的说法吗？“Pixels are organized row-wise.”，就是说它是按行来排布数据的。现在的 images 是一个大的单列数组（column vector），这相当于数据被转置了 $90^{\circ} $ ，我们暂时先搁置这个问题。
...
通过 `reshape` 函数，我们将数据塑型为一个 28 x 28 x 60000 的 3 维数组。形象的比喻是，此时的数据调制为一叠 60000 张的卡片，每张卡片上印刷有一张 28 x 28 个像素的图片。让我们抽出其中的第 128 张卡片来看看是什么东西：
...
很容易想到的，就是将我们上述在 octave 环境下的交互操作写成为一个函数。实际上，这些交互操作，也是我从网上找的一些开源项目的源代码中整理的。我演示的这些操作，都是为了方便说明问题来组织的。在真实的工程项目中，情况可能会精细很多。譬如，可以考虑支持机器学习的小批量（mini-patch）训练的加载方式等等。即使是现在的这个 3 维数组的数据组织形式，也是可以根据未来项目发展来修改的。